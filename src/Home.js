...
// Home.js
import React, { useState, useEffect, useRef } from "react";
import "./Home.css"; // See next section for recommended styles
...
function Home() {
  ...
  const handleCommand = (cmd) => {
    switch (cmd.toLowerCase()) {
      ...
      case "about":
        return ["I'm a software developer building cool stuff!"];
      case "clear":
        return null; // we'll handle actual clearing in onSubmit
      default:
        if (!cmd.trim()) return [];
        return [`'${cmd}' is not recognized as a valid command.`];
    }
  ...
  const onSubmitCommand = (e) => {
    ...
    // 1) Show the typed command
    setLines((prev) => [...prev, `C:\\Users\\Recruiter> ${command}`]);

    // 2) Get output from handleCommand
    const output = handleCommand(command);

    // 3) If 'clear', reset lines
    if (command.toLowerCase() === "clear") {
      setLines([]);
    } else if (output && output.length > 0) {
      // 4) Append returned lines
      setLines((prev) => [...prev, ...output]);
    }
